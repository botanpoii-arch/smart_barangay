...
import { fetchResidentsAPI, fetchOfficialsAPI, saveBlotterAPI } from './Api'; // IMPORTED API CALLS
...
export const FileComponent: React.FC<IFileProps> = ({ onClose, onRefresh, selectedCase }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const captain = offData.find((o: IOfficial) => o.position === 'Barangay Captain' && o.status === 'Active');
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      // USING THE NEW SECURE API HANDSHAKE CALL
      const savedData = await saveBlotterAPI(formData.id, submissionData);
